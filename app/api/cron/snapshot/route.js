...
export async function GET(req) {
  ...
  const now = new Date()
  const day = now.getDay() // 4=Thu, 5=Fri, 6=Sat, 0=Sun
  const roundMap = { 4: 1, 5: 2, 6: 3, 0: 4 }
  const round = roundMap[day]

  if (!round) return Response.json({ error: 'Not a tournament day' }, { status: 400 })

  return await runSnapshot(round)
}

export async function runSnapshot(round) {
    ...
    const golferScores = golfers.map(g => {
      ...
      if (!espn) {
        ...
        if (replacement) {
          ...
          totalScore += repEspn.score; todayScore += repEspn.today
